...
local esp = loadstring(game:HttpGet("https://raw.githubusercontent.com/Eternal1ty/MainScripts/main/Esp.lua"))() 
esp:init()
...
setfpscap(getgenv().maxfps or 250)
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText(MenuName or "etrx.al", true, false, 6, 6, bbmenu)
								...
								if v2.extra ~= nil then
									if v2.extra.type == KEYBIND then
										...
										event:connect(function(newval) 
											if menu:GetVal("Visuals", "Misc" ,"Log Keybinds") then 
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "etrx.al"
...
if menu.game == "uni" then --SECTION UNIVERSAL
					...
					content = {
						{
							type = TOGGLE,
							name = "Enabled",
							tooltip = "Enables Bullet Redirection",
							value = false,
						},
                        {
							type = DROPBOX,
					...
					content = {
						{
							...
							tooltip = "Extends Players Head Hitbox",
							value = false,
						},
						{
							type = SLIDER,
							name = "Hitbox Extend Ammount",
							value = 0,
							minvalue = 0,
							maxvalue = 6,
							stradd = "%",
			...
			content = {
				{
					...
					content = {
						...
						},
                        {
							...
							extra = {
								type = COLORPICKER,
								name = "Chams Inline Color",
								color = { 170,170,170, 191.25 },
							},
						},
                        {
							type = TOGGLE,
				...
				},
                {
                    ...
                    content = {
						...
						},
                        {
							...
							name = "Text Font",
							value = 2,
							values = { "Monospace", "Plex", "System", "UI"},
						},
                        {
							type = SLIDER,
							name = "Text Size",
				...
				},
                {
					...
					content = {
						...
						},
                        {
							...
							extra = {
								type = DOUBLE_COLORPICKER,
								name = { "Outdoor Ambiance", "Indoor Ambiance" },
						...
						},
						{
							...
							name = "Force Time",
							value = false,
							tooltip = "Forces the time to the time set below.",
						},
						{
							type = SLIDER,
							name = "Custom Time",
						...
						},
						{
							...
							name = "Lighting Type",
							value = 1, 
							values = { "Future", "Voxel", "ShadowMap", "Compatibility" },
						},
					}
				},
				{
	...
	local function find_2d_distance ( pos1, pos2 ) -- found this func here https://love2d.org/forums/viewtopic.php?t=1951 ty to whoever did this
		local dx = pos1.X - pos2.X
		local dy = pos1.Y - pos2.Y
		return math.sqrt ( dx * dx + dy * dy )
	end

	function IsVisible(character, position, arg)
		--local origin = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
		local origin = workspace.CurrentCamera.CFrame.Position
		local part = workspace.FindPartOnRayWithIgnoreList(workspace, Ray.new(origin, position - origin), {game.Players.LocalPlayer.Character, Camera, character}, false, true)
		return part == nil
	end
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Teleport" then
			if selectedPlayer == nil then
				CreateNotification("No player selected")
			elseif selectedPlayer == LOCAL_PLAYER then
				CreateNotification("You can't teleport to yourself silly ")
			else
				...
				if character then
					...
					local tpn = "none" -- this means teleported part name,,, for debug
					for index, partname in ipairs({"Head", "Toros", "UpperTorso", "HumanoidRootPart", "RootPart"}) do
						targetPos = character:FindFirstChild(partname)
						if targetPos then 
							tpn = partname
							break
						end
					...
					if targetPos then
						local targetRelPos = targetPos.Position
						if targetRelPos then
							
							local RP = LOCAL_PLAYER.Character.HumanoidRootPart
							RP.CFrame = CFrame.new(targetRelPos + Vector3.new(0, 7, 0))
							CreateNotification("Teleported to \"".. selectedPlayer.Name.. "\" using part: ".. tpn )
						else
							CreateNotification("Failed teleport, couldnt't find position")
						end
					else
						CreateNotification("Failed teleport, couldn't find a suitable part")
					end
				else
					CreateNotification("Failed teleport, couldn't find player character")
				end
			end
		end
	end)
	...
	menu.connections.renderstepped2 = game.RunService.RenderStepped:Connect(function()
		...
		if menu:GetVal("Visuals", "Camera Visuals", "Change FOV") then
			Camera.FieldOfView = menu:GetVal("Visuals", "Camera Visuals", "Camera FOV")
		end
		if menu:GetVal("Visuals", "Camera Visuals", "Change Max Zoom") then
			LOCAL_PLAYER.CameraMaxZoomDistance = menu:GetVal("Visuals", "Camera Visuals", "Max Zoom Distance")
		else
			LOCAL_PLAYER.CameraMaxZoomDistance = 20
		end
		if menu:GetVal("Visuals", "Camera Visuals", "Zoom") then
			local keybind = menu:GetVal("Visuals", "Camera Visuals", "Zoom", KEYBIND)
			local held = INPUT_SERVICE:IsKeyDown(keybind)

			if held and menu:GetVal("Visuals", "Camera Visuals", "Zoom") then
			    Camera.FieldOfView = menu:GetVal("Visuals", "Camera Visuals", "Zoom Distance")
			end
		end

		if game.Lighting ~= nil then
		    sethiddenproperty(game.Lighting, "Technology", menu:GetVal("Visuals", "World Visuals", "Lighting Type"))
            ...
            if menu:GetVal("Visuals", "World Visuals", "Ambience") then
                game.Lighting.Ambient = menu:GetVal("Visuals", "World Visuals", "Ambience", COLOR2, true)
                game.Lighting.OutdoorAmbient = menu:GetVal("Visuals", "World Visuals", "Ambience", COLOR1, true)
			end

            if menu:GetVal("Visuals", "World Visuals", "Custom Saturation") then
                ColorCorrection.Enabled = true
                ColorCorrection.TintColor = menu:GetVal("Visuals", "World Visuals", "Custom Saturation", COLOR, true)
                ColorCorrection.Saturation = menu:GetVal("Visuals", "World Visuals", "Saturation Density") / 50
            else
			...
			if menu:GetVal("Visuals", "World Visuals", "Global Shadows") then
...
CreateNotification(string.format("Done loading the cheat. (%d ms)", menu.load_time))
CreateNotification("Press DELETE to open and close the menu!")
CreateNotification("Report any bugs to etrxal#6368")
...
--esp
 menu.connections.renderstepped3 = game.RunService.Heartbeat:Connect(function()
    esp.enabled = menu:GetVal("Players", "Player ESP", "Enabled")

    esp.textlayout.Name.Enabled      = menu:GetVal("Players", "Player ESP", "Name")
    esp.textlayout.Name.Color        = menu:GetVal("Players", "Player ESP", "Name", COLOR, true)
    esp.textlayout.Name.Transparency = menu:GetVal("Players", "Player ESP", "Name", COLOR)[4]/255

    esp.box[1] = menu:GetVal("Players", "Player ESP", "Bounding Box")
    esp.box[2] = menu:GetVal("Players", "Player ESP", "Bounding Box", COLOR, true)
    esp.box[3] = menu:GetVal("Players", "Player ESP", "Bounding Box", COLOR)[4]/255

    esp.boxfill[1] = menu:GetVal("Players", "Player ESP", "Bounding Box Fill")
    esp.boxfill[2] = menu:GetVal("Players", "Player ESP", "Bounding Box Fill", COLOR, true)
    esp.boxfill[3] = menu:GetVal("Players", "Player ESP", "Bounding Box Fill", COLOR)[4]/255

    esp.barlayout.Health.Enabled = menu:GetVal("Players", "Player ESP", "Health Bar")
    esp.barlayout.Health.Color2  = menu:GetVal("Players", "Player ESP", "Health Bar", COLOR2, true)
    esp.barlayout.Health.Color1  = menu:GetVal("Players", "Player ESP", "Health Bar", COLOR1, true)

    esp.textlayout.Health.Enabled    = menu:GetVal("Players", "Player ESP", "Health Number")

    esp.textlayout.Tool.Enabled      = menu:GetVal("Players", "Player ESP", "Tool")
    esp.textlayout.Tool.Color        = menu:GetVal("Players", "Player ESP", "Tool", COLOR, true)
    esp.textlayout.Tool.Transparency = menu:GetVal("Players", "Player ESP", "Tool", COLOR)[4]/255

    esp.textlayout.Distance.Enabled      = menu:GetVal("Players", "Player ESP", "Distance")
    esp.textlayout.Distance.Color        = menu:GetVal("Players", "Player ESP", "Distance", COLOR, true)
    esp.textlayout.Distance.Transparency = menu:GetVal("Players", "Player ESP", "Distance", COLOR)[4]/255

    esp.tracer[1] = menu:GetVal("Players", "Player ESP", "Tracer")
    esp.tracer[2] = menu:GetVal("Players", "Player ESP", "Tracer", COLOR, true)
    esp.tracer[3] = menu:GetVal("Players", "Player ESP", "Tracer", COLOR)[4]/255

    esp.angle[1] = menu:GetVal("Players", "Player ESP", "View Angle")
    esp.angle[2] = menu:GetVal("Players", "Player ESP", "View Angle", COLOR, true)
    esp.angle[3] = menu:GetVal("Players", "Player ESP", "View Angle", COLOR)[4]/255

    esp.arrow[1] = menu:GetVal("Players", "Player ESP", "Arrow")
    esp.arrow[2] = menu:GetVal("Players", "Player ESP", "Arrow", COLOR, true)
    esp.arrow[3] = menu:GetVal("Players", "Player ESP", "Arrow", COLOR)[4]/255

    esp.arrowradius = menu:GetVal("Players", "Player ESP", "Arrow Radius")

    esp.limitdistance = menu:GetVal("Players", "ESP Settings", "Limit Draw Distance")
    esp.maxdistance   = menu:GetVal("Players", "ESP Settings", "Draw Distance Limit")

    esp.displayname   = menu:GetVal("Players", "ESP Settings", "Use Display Name")
    esp.textfont      = menu:GetVal("Players", "ESP Settings", "Text Font")
    esp.textsize      = menu:GetVal("Players", "ESP Settings", "Text Size")


    if menu:GetVal("Players", "Player ESP", "Bounding Box Type") == 1 then
        esp.custombox = "default"
    else
        esp.custombox = "corner"    
    end



    if menu:GetVal("Players", "ESP Settings", "Name Position") == 1 then
        esp.textlayout.Name.Position = "top"
    elseif menu:GetVal("Players", "ESP Settings", "Name Position") == 2 then
        esp.textlayout.Name.Position = "left"
    elseif menu:GetVal("Players", "ESP Settings", "Name Position") == 3 then
        esp.textlayout.Name.Position = "right"
    else 
        esp.textlayout.Name.Position = "bottom"
    end

    if menu:GetVal("Players", "ESP Settings", "Healthbar Position") == 1 then
        esp.barlayout.Health.Position = "top"
    elseif menu:GetVal("Players", "ESP Settings", "Healthbar Position") == 2 then
        esp.barlayout.Health.Position = "left"
    elseif menu:GetVal("Players", "ESP Settings", "Healthbar Position") == 3 then
        esp.barlayout.Health.Position = "right"
    else 
        esp.barlayout.Health.Position = "bottom"
    end

    if menu:GetVal("Players", "ESP Settings", "Health Number Position") == 1 then
        esp.textlayout.Health.Position = "top"
    elseif menu:GetVal("Players", "ESP Settings", "Health Number Position") == 2 then
        esp.textlayout.Health.Position = "left"
    elseif menu:GetVal("Players", "ESP Settings", "Health Number Position") == 3 then
        esp.textlayout.Health.Position = "right"
    else 
        esp.textlayout.Health.Position = "bottom"
    end

    if menu:GetVal("Players", "ESP Settings", "Distance Position") == 1 then
        esp.textlayout.Distance.Position = "top"
    elseif menu:GetVal("Players", "ESP Settings", "Distance Position") == 2 then
        esp.textlayout.Distance.Position = "left"
    elseif menu:GetVal("Players", "ESP Settings", "Distance Position") == 3 then
        esp.textlayout.Distance.Position = "right"
    else 
        esp.textlayout.Distance.Position = "bottom"
    end
...
menu.connections.renderstepped4 = game.RunService.Heartbeat:Connect(function()
    if menu:GetVal("Combat", "Combat Exploits", "Hitbox Extender") then
        for i, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Head") then
                v.Character.Head.Size         = Vector3.new(1,1,1) * menu:GetVal("Combat", "Combat Exploits", "Hitbox Extend Ammount")
        ...
        for i, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Head") then
                v.Character.Head.Size         = NormalHeadSize
